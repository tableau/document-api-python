...
class ConnectionModelTests(unittest.TestCase):
    ...
    def test_can_write_attributes_to_connection(self):
        ...
        conn.authentication = 'wibble'
        conn.dbclass = 'invalid'
        self.assertEqual(conn.dbname, 'BubblesInMyDrink')
        self.assertEqual(conn.username, '')
        self.assertEqual(conn.server, 'mssql2014.test.tsi.lan')
        self.assertEqual(conn.authentication, 'wibble')
        self.assertEqual(conn.dbclass, 'invalid')
...
class DatasourceModelV10Tests(unittest.TestCase):
    ...
    def test_find_matching_connection_by_attributes_same(self):
        ds = Datasource.from_file(self.tds_file.name)
    ...
    def test_find_matching_connection_by_attributes_same(self):
        ...
        self.assertEqual(len(my_connections), 2)
        self.assertEqual(my_connections[0].dbclass, 'mysql')
        self.assertEqual(my_connections[1].server, 'mssql2012.test.tsi.lan')
